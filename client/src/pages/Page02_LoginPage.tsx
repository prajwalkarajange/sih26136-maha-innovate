  ...
  return <div className="min-h-screen bg-[#f7f6f2] text-[#102f52] font-sans">
    <main className="grid min-h-[calc(100vh-72px)] lg:grid-cols-[55%_45%]">
      ...
      <section className="flex items-start justify-center px-4 py-10 sm:px-8 lg:items-center lg:py-8"><div className="w-full max-w-[455px] rounded-md border border-slate-200 bg-white p-6 shadow-[0_8px_28px_rgba(16,47,82,.09)] sm:p-7">
        ...
        {!isRegister && <div className="mb-5 grid grid-cols-4 overflow-hidden rounded-md border border-slate-200">{roles.map(({ value, label, icon: Icon }) => <button key={value} type="button" onClick={() => selectRole(value)} className={`min-h-[56px] px-1.5 text-xs font-semibold transition ${role === value ? 'bg-[#0b2f59] text-white' : 'bg-white text-[#29435e] hover:bg-slate-50'}`}><Icon className="mx-auto mb-1 h-[18px] w-[18px]" /><span className="block leading-tight">{label}</span></button>)}</div>}
        ...
        <form onSubmit={handleFormSubmit} className="space-y-4">
          ...
          <div><label className="mb-1.5 block text-sm font-bold">Email ID</label><div className="relative"><Mail className="absolute left-3 top-3 h-4 w-4 text-[#0b2f59]" /><input type="text" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="Enter your official email ID" className="h-11 w-full rounded border border-slate-300 pl-9 pr-3 text-sm outline-none focus:border-[#0b2f59]" /></div></div>
          <div><label className="mb-1.5 block text-sm font-bold">Password</label><div className="relative"><Lock className="absolute left-3 top-3 h-4 w-4 text-[#0b2f59]" /><input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required placeholder="Enter your password" className="h-11 w-full rounded border border-slate-300 pl-9 pr-10 text-sm outline-none focus:border-[#0b2f59]" /><button type="button" aria-label={showPassword ? 'Hide password' : 'Show password'} onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-2.5 text-[#0b2f59]">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></div>
          ...
          <button type="submit" disabled={loading} className="h-11 w-full rounded bg-[#0b3765] text-base font-bold text-white transition hover:bg-[#082b50] disabled:cursor-wait disabled:opacity-70">{loading ? 'Authenticating...' : isRegister ? 'Register Startup' : 'Login'}</button>
        </form>
        {!isRegister && <><div className="my-4 flex items-center gap-3 text-xs text-slate-400"><span className="h-px flex-1 bg-slate-200" />OR<span className="h-px flex-1 bg-slate-200" /></div><button type="button" className="h-11 w-full rounded border border-slate-300 text-sm font-bold text-[#1e3550]"><span className="mr-2 text-base font-bold text-[#4285f4]">G</span>Continue with Google</button><p className="mt-4 text-center text-xs text-slate-500">Don't have an account? Contact your department administrator.</p></>}
        <div className="mt-5 rounded-md bg-[#eef4fa] p-3 text-sm text-[#29435e]"><div className="flex gap-3"><Shield className="mt-0.5 h-5 w-5 shrink-0 text-[#0b3765]" /><div><strong>This portal is for authorized users only.</strong><br />All activities are monitored and secured.</div></div></div>
        <button type="button" onClick={() => setShowDemoLogins(!showDemoLogins)} className="mt-5 flex w-full items-center justify-between border-t border-slate-200 pt-4 text-sm font-bold text-[#0b3765]"><span>Demo Logins</span><span>{showDemoLogins ? '−' : '+'}</span></button>
        ...
        <div className="mt-4 border-t border-slate-100 pt-3 text-center text-xs text-slate-500">{isRegister ? <button type="button" onClick={() => setIsRegister(false)} className="font-bold text-blue-600 hover:underline">Already registered? Login Now</button> : <button type="button" onClick={() => setIsRegister(true)} className="h-10 w-full rounded border border-[#d94b0b] bg-[#f05a16] px-4 text-sm font-bold text-white transition hover:bg-[#d94b0b]">New Startup? Register Now</button>}</div>
      </div></section>
    </main>
